...
structure Context where
  ...
  /-- The heartbeat number that `maxHeartbeats` counts from. Setting this to `0` amounts to
  counting heartbeats since the thread was started. -/
  initHeartbeats : Nat
...
def ImportM.runCoreM (x : CoreM α) : ImportM α := do
  let ctx ← read
  let (a, _) ← (withOptions (fun _ => ctx.opts) x).toIO
    { fileName := "<ImportM>", fileMap := default
